...
It can be deployed like this:
    ...
    oc apply -f install_and_examples_0/install/cluster-operator/ -n amq-streams-demo
         or
    oc apply -f https://raw.githubusercontent.com/skoussou/ocp-amq-streams-workshop/master/install_and_examples_0/install/cluster-operator/ -n amq-streams-demo
...
* This is left as a possible *TODO* as it requires a lot of workload to produce meaningful results. For the possible settings link:https://docs.openshift.com/container-platform/3.11/admin_guide/scheduling/pod_affinity.html[read this documentation]
...
**This functionality only works when nodes can be tainted. For more link:https://docs.openshift.com/container-platform/3.11/admin_guide/scheduling/taints_tolerations.html[read docs]**
...
See:
* link:https://access.redhat.com/documentation/en-us/red_hat_amq/7.2/html-single/using_amq_streams_on_openshift_container_platform/#assembly-kafka-rack-deployment-configuration-kafka[Kafka rack awareness]
* link:Cluster Operator Configuration[STRIMZI_DEFAULT_KAFKA_INIT_IMAGE]
...
* Scale consumer down to 0 replicas
	...
	oc scale deployment hello-world-consumer --replicas=0 -n amq-streams-demo

* Check the PVCs assined due to Openshift StatefulSets to each pod

	oc get pvc -n amq-streams-demo

* Scale producer down to 0 replicas

	oc scale deployment hello-world-producer --replicas=0 -n amq-streams-demo

* After awhile we scale down the KAFKA nodes to 0 replicas

	oc scale statefulset my-cluster-kafka --replicas=0 -n amq-streams-demo

* Scale up  the KAFKA nodes to 2 replicas

	oc scale statefulset my-cluster-kafka --replicas=2

* Scale up ONLY the consumer

	oc scale deployment hello-world-consumer --replicas=1 -n amq-streams-demo

* messages not read will be processed
...
Check the Kafka Connect logs to see the messages arriving.
    ...
    oc logs $(oc get pod -l app=amq-streams-demo-connect -o=jsonpath='{.items[0].metadata.name}') -f
